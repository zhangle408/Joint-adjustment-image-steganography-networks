...
from models.HidingUNet_3_coarse_190 import UnetGenerator_coarse
from models.HidingUNet_3_fine_190 import UnetGenerator_fine
from models.HidingUNet_3_fine_1 import UnetGenerator_fine_1
from models.RevealNet_C import RevealNet_C
from models.RevealNet_S import RevealNet_S
from models.RevealUNet_3_190 import UnetReveal_3
...
def main():
    ...
    if opt.cover_dependent:
        Hnet_C = UnetGenerator_coarse(input_nc=opt.channel_secret * opt.num_secret + opt.channel_cover * opt.num_cover,
                                      ...
                                      use_tanh=0)
        Hnet_F1 = UnetGenerator_fine_1(input_nc=opt.channel_secret * opt.num_secret + opt.channel_cover * opt.num_cover,
                                       output_nc=opt.channel_cover * opt.num_cover, num_downs=num_downs,
                                       norm_layer=norm_layer,
                                       use_tanh=0)
        Hnet_F2 = UnetGenerator_fine(input_nc=opt.channel_secret * opt.num_secret + opt.channel_cover * opt.num_cover,
                                     output_nc=opt.channel_cover * opt.num_cover, num_downs=num_downs,
                                     norm_layer=norm_layer,
                                     use_tanh=0)
        ...
        Hnet_C = UnetGenerator_coarse(input_nc=opt.channel_secret * opt.num_secret,
                                      ...
                                      norm_layer=norm_layer,
                                      use_tanh=1)
        Hnet_F1 = UnetGenerator_fine_1(input_nc=opt.channel_secret * opt.num_secret,
                                       output_nc=opt.channel_cover * opt.num_cover, num_downs=num_downs,
                                       norm_layer=norm_layer,
                                       use_tanh=1)
        Hnet_F2 = UnetGenerator_fine(input_nc=opt.channel_secret * opt.num_secret,
                                     output_nc=opt.channel_cover * opt.num_cover, num_downs=num_downs,
                                     norm_layer=norm_layer,
                                     use_tanh=1)

    Rnet_C1 = RevealNet_C(input_nc=opt.channel_cover * opt.num_cover, output_nc=opt.channel_cover * opt.num_cover,
                          nhf=64,
                          norm_layer=norm_layer, output_function=nn.Sigmoid)
    Rnet_S1 = RevealNet_S(input_nc=opt.channel_cover * opt.num_cover, output_nc=opt.channel_secret * opt.num_secret,
                          nhf=64,
                          norm_layer=norm_layer, output_function=nn.Sigmoid)
    Rnet_C2 = RevealNet_C(input_nc=opt.channel_cover * opt.num_cover, output_nc=opt.channel_cover * opt.num_cover,
                          nhf=64,
                          norm_layer=norm_layer, output_function=nn.Sigmoid)
    Rnet_S2 = RevealNet_S(input_nc=opt.channel_cover * opt.num_cover, output_nc=opt.channel_secret * opt.num_secret,
                          nhf=64,
                          norm_layer=norm_layer, output_function=nn.Sigmoid)
    RUnet_3 = UnetReveal_3(input_nc=opt.channel_cover * opt.num_cover, output_nc=opt.channel_secret * opt.num_secret,
                           num_downs=num_downs,
                           norm_layer=norm_layer,
                           use_tanh=0)
    ...
    Hnet_F1 = torch.nn.DataParallel(Hnet_F1).cuda()
    Rnet_C1 = torch.nn.DataParallel(Rnet_C1).cuda()
    Rnet_S1 = torch.nn.DataParallel(Rnet_S1).cuda()
    Hnet_F2 = torch.nn.DataParallel(Hnet_F2).cuda()
    Rnet_C2 = torch.nn.DataParallel(Rnet_C2).cuda()
    Rnet_S2 = torch.nn.DataParallel(Rnet_S2).cuda()
    ...
    if opt.checkpoint != "":
        ...
        Hnet_F1.load_state_dict(checkpoint['H_F1_state_dict'])
        Rnet_C1.load_state_dict(checkpoint['R_C1_state_dict'])
        Rnet_S1.load_state_dict(checkpoint['R_S1_state_dict'])
        Hnet_F2.load_state_dict(checkpoint['H_F2_state_dict'])
        Rnet_C2.load_state_dict(checkpoint['R_C2_state_dict'])
        Rnet_S2.load_state_dict(checkpoint['R_S2_state_dict'])
    ...
    if opt.test == '':
        ...
        params = list(Hnet_C.parameters()) + list(Rnet_C1.parameters()) + list(Rnet_S1.parameters()) + list(
            Rnet_C2.parameters()) + list(Rnet_S2.parameters()) + list(RUnet_3.parameters()) + list(
            Hnet_F1.parameters()) + list(Hnet_F2.parameters())
        ...
        for epoch in range(opt.epochs):
            ...
            ######################## train ##########################################
            train(train_loader, epoch, Hnet_C=Hnet_C, Hnet_F1=Hnet_F1, Rnet_C1=Rnet_C1, Rnet_S1=Rnet_S1, Hnet_F2=Hnet_F2, Rnet_C2=Rnet_C2, Rnet_S2=Rnet_S2, RUnet_3=RUnet_3, criterion=criterion)

            ####################### validation  #####################################
            val_hloss, val_rloss, val_hdiff, val_rdiff = validation(val_loader, epoch, Hnet_C=Hnet_C, Hnet_F1=Hnet_F1, Rnet_C1=Rnet_C1, Rnet_S1=Rnet_S1, Hnet_F2=Hnet_F2, Rnet_C2=Rnet_C2, Rnet_S2=Rnet_S2, RUnet_3=RUnet_3,
                                                                    criterion=criterion)
            ...
            save_checkpoint({
                ...
                'H_F1_state_dict': Hnet_F1.state_dict(),
                'R_C1_state_dict': Rnet_C1.state_dict(),
                'R_S1_state_dict': Rnet_S1.state_dict(),
                'H_F2_state_dict': Hnet_F2.state_dict(),
                'R_C2_state_dict': Rnet_C2.state_dict(),
                'R_S2_state_dict': Rnet_S2.state_dict(),
    ...
    # For testing the trained network
        ...
        analysis(test_loader, 0, Hnet_C=Hnet_C, Hnet_F1=Hnet_F1, Rnet_C1=Rnet_C1, Rnet_S1=Rnet_S1, Hnet_F2=Hnet_F2, Rnet_C2=Rnet_C2, Rnet_S2=Rnet_S2, criterion=criterion)
...
def forward_pass(secret_img, secret_target, cover_img, cover_target, Hnet_C, Hnet_F1, Rnet_C1, Rnet_S1, Hnet_F2, Rnet_C2, Rnet_S2, RUnet_3, criterion, val_cover=0, i_c=None,
...
#+++++++++++++++++++++++++++
    ...
    rev_secret_img1 = Rnet_S1(container_img)
    rev_cover_img1 = Rnet_C1(container_img)
    deta_secret1 = secret_imgv_nh - rev_secret_img1
    deta_cover1 = cover_imgv - rev_cover_img1
    F_input1 = torch.cat((deta_cover1, deta_secret1), dim=1)
    # container_31, container_21, final_container1 = Hnet_F1(F_input1, out_1, out_2, out_3, out_4, out_5, itm_secret_img)
    out_1_1, out_2_1, out_3_1, out_4_1, out_5_1, final_container1 = Hnet_F1(F_input1, out_1, out_2, out_3, out_4, out_5,
                                                                            itm_secret_img)
    rev_secret_img2 = Rnet_S2(final_container1)
    rev_cover_img2 = Rnet_C2(final_container1)
    deta_secret2 = secret_imgv_nh - rev_secret_img2
    deta_cover2 = cover_imgv - rev_cover_img2
    F_input2 = torch.cat((deta_cover2, deta_secret2), dim=1)
    container_3, container_2, final_container = Hnet_F2(F_input2, out_1_1, out_2_1, out_3_1, out_4_1, out_5_1,
                                                        final_container1)
...
def train(train_loader, epoch, Hnet_C, Hnet_F1, Rnet_C1, Rnet_S1, Hnet_F2, Rnet_C2, Rnet_S2, RUnet_3, criterion):
...
#cover_imgv, final_container, secret_imgv_nh, final_rev_secret, errH, errH_3, errH_2, errR, errR_3, errR_2, diffH, diffH_3, diffH_2, diffR, diffR_3, diffR_2
        cover_imgv, final_container, secret_imgv_nh, final_rev_secret, errH, errH_3, errH_2, errR, errR_3, errR_2, diffH, diffH_3, diffH_2, diffR, diffR_3, diffR_2 \
            = forward_pass(secret_img, secret_target, cover_img, cover_target, Hnet_C, Hnet_F1, Rnet_C1, Rnet_S1, Hnet_F2, Rnet_C2, Rnet_S2, RUnet_3, criterion)
...
def validation(val_loader, epoch, Hnet_C, Hnet_F1, Rnet_C1, Rnet_S1, Hnet_F2, Rnet_C2, Rnet_S2, RUnet_3, criterion):
    print(
        "#################################################### validation begin ########################################################")
    start_time = time.time()
    Hnet_C.eval()
    ...
    for i, ((secret_img, secret_target), (cover_img, cover_target)) in enumerate(val_loader, 0):
        ...
        cover_imgv, final_container, secret_imgv_nh, final_rev_secret, errH, errH_3, errH_2, errR, errR_3, errR_2, diffH, diffH_3, diffH_2, diffR, diffR_3, diffR_2 \
            = forward_pass(secret_img, secret_target, cover_img, cover_target, Hnet_C, Hnet_F1, Rnet_C1, Rnet_S1, Hnet_F2, Rnet_C2, Rnet_S2, RUnet_3, criterion, val_cover=1)
